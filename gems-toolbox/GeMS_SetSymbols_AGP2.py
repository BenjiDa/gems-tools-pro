...
def test_locks(gdb, n):
    ...
    if read_locks:
        ...
        addMsgAndPrint('Close it if open in ArcGIS Pro or compact the '
    ...
    if ed_locks:
        ...
        addMsgAndPrint('Close the edit session if open in ArcGIS Pro or compact the '
                        'geodatabase if being run from the command line ')
        sys.exit(1)
...
if inFds.find('CorrelationOfMapUnits') == -1:
    ...
    fields = ['Type','IsConcealed', 'LocationConfidenceMeters', 'ExistenceConfidence', 'IdentityConfidence', 'Symbol', 'OBJECTID']
else:  # is CMU
    caf = ''
        ...
        with arcpy.da.UpdateCursor(fc, fields) as cursor:
            for row in cursor:
                ...
                if typ in list(EightfoldLineDict.keys()):
                    if debug1: addMsgAndPrint('{} is in EightfoldLineDict'.format(typ))
                    inc = 0                     
                ...
                if rowChanged:
                    ...
                    if top_bool: edit.startOperation()
                    cursor.updateRow(row)
                    if top_bool: edit.stopOperation()
                    
        if top_bool: edit.stopEditing(True)
        if top_bool: del edit   
