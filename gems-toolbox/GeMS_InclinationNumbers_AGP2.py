...
def insert_layer(lsource, new_lyr_file):
    if debug1: addMsgAndPrint('lsource = {}'.format(lsource))
        ...
        for lyr in lList:
            ...
            if lyr.supports('isFeatureLayer'):
                if lyr.isFeatureLayer:
                    if debug1: writeLayerNames(lyr)
                    if lyr.dataSource == lsource:
                        m.insertLayer(lyr, new_lyr_file, "BEFORE")
                        
def writeLayerNames(lyr):
    if lyr.supports('name'): addMsgAndPrint('       name: {}'.format(lyr.name))
    if lyr.supports('connectionProperties'): arcpy.AddMessage(lyr.connectionProperties)
...
def dipNumbers(gdb,mapScaleDenominator):
    ...
    if not arcpy.Exists(OPfc):
        addMsgAndPrint('  Geodatabase {} lacks feature class OrientationPoints.'
          .format(os.path.basename(gdb)))
        return
